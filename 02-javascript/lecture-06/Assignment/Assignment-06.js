...
for(let i=1; i<=3; i++){
    let withdrawals = prompt("Enter the amount to Withdrawal")
    amount = Number(withdrawals)

    if(withdrawals === null){
        console.error("Enter the cancle button");
        break;
    }
    
    if(withdrawals.trim() ===""){
        console.error("You enetr some thing wrong")
    }

    if(amount<=balance){
        balance-=amount;
        console.log("money withdraw succfully");
